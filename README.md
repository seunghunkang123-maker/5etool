...
| 서식 | 카드 본문은 전체 리치 텍스트. 특성·행동·장비·인벤토리 등 짧은 칸에도 굵게·기울임·밑줄·목록을 쓸 수 있다 |
| 기타 | 통합 검색, 이미지/핸드아웃 업로드, 세션 로그·되돌리기, 감사 로그, 알림 |
...
npm run test        26개 파일 / 302개 테스트 통과
npm run build       통과 (코드 분할된 청크로 빌드)
npm run test:e2e    5개 시나리오 통과 (전체 세션 흐름 / 설정 / 상태 도감 /
                    보관함·세션 관리 / 특성·장비 서식)
./supabase/test/run_checks.sh
